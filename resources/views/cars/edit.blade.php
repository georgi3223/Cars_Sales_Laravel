@extends('layouts.app') @section('content') <div class="sm:w-1/2 m-10 mx-auto">
  <form method="POST" action="{{ route('cars.update', $car->id) }}" enctype="multipart/form-data" class='bg-white shadow-md rounded p-10 w-full'> @csrf @method('PUT')
    <!-- Display validation errors if any --> @if ($errors->any()) <div class="alert alert-danger">
      <ul> @foreach ($errors->all() as $error) <li>{{ $error }}</li> @endforeach </ul>
    </div> @endif <h2 class="text-2xl font-semibold mb-4">Edit Ad</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <div class="mb-4">
        ...
        <select name="year" id="year" class="form-select block w-full py-2 px-3 border border-gray-300 rounded focus:outline-none focus:ring-blue-500 focus:border-blue-500">
          <option value="" selected hidden>Select year</option> <?php
        $currentYear = date('Y');
        $startYear = 1900; // Specify the start year here
    ...
    <div class="mb-4">
      ...
      <input type="file" name="image1" id="image1" class="form-input rounded-md shadow-sm mt-1 block ">
    </div>
    <div class="mb-4">
      <label for="image2" class="block text-sm font-medium text-gray-700">Image 2</label>
      <input type="file" name="image2" id="image2" class="form-input rounded-md shadow-sm mt-1 block ">
    </div>
    <div class="mb-4">
      <label for="image3" class="block text-sm font-medium text-gray-700">Image 3</label>
      <input type="file" name="image3" id="image3" class="form-input rounded-md shadow-sm mt-1 block ">
    </div>
    <div class="mb-4">
      <label for="image4" class="block text-sm font-medium text-gray-700">Image 4</label>
      <input type="file" name="image4" id="image4" class="form-input rounded-md shadow-sm mt-1 block">
    </div>
    <div class="mb-4">
    ...
    <!-- Submit Button -->
    <div class="mb-4">
      <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-md">Edit Ad</button>
    </div>
  </form>
</div> @endsection